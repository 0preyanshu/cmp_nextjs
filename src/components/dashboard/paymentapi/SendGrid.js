...
export function SendGrid() {
  ...
  const paypalData = useSelector((state) => state?.companyPaymentDetails?.paypalData);
  const { updatePaypalCompanyPaymentDetails,getCompanyPaymentDetails } = CompanyPaymentDetailsActions;
  ...
  const defaultValues = React.useMemo(
    () => ({
      DevPublishableKey: paypalData?.DevPublishableKey || '',
      DevSecretKey: paypalData?.DevSecretKey || '',
      LivePublishableKey: paypalData?.LivePublishableKey || '',
      LiveSecretKey: paypalData?.LiveSecretKey || '',
    }),
    [paypalData]
  );
  ...
  const onSubmit = React.useCallback(
    async (data) => {
      try {
        ...
        await dispatch(updatePaypalCompanyPaymentDetails(newobject)).then((res) => {
